...
const getCx = async (
  ...
  return cxVersion == '2' ? await ndexClient.getCX2Network(uuid) : await ndexClient.getRawNetwork(uuid)
}
...
export default function useCx(
  ...
  return useQuery(['cx', uuid, serverUrl, apiVersion, credential, threshold, objectCount, cxVersion], () =>
    getCx(uuid, serverUrl, apiVersion, credential, threshold, objectCount, cxVersion),
  )
}
